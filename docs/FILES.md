...
| `store.py` | CRUD over SQLite: query, deep insert, update with MERGE-vs-PUT semantics, cascading delete, auto-assigned document numbers, item numbering, total recalculation, ABAP initial values, and the monotonic change timestamp the ETag is derived from. | Changing write behaviour or the system-filled fields. |
