...
| `metadata4.py` | CSDL 4.0, in XML and in JSON, including the UI and Capabilities annotations and the vocabulary references that make them resolve. No Association elements - V4 navigation properties carry their target type, and the container binds them to entity sets. |
...
| `annotations.py` | The V2 annotation document: the same `EntityType.ui` intent the V4 services render inside `$metadata`, in the separate document a V2 app fetches. Only a service that opts in publishes one. |
...
| `documents.py` | Creating the documents a sales order turns into - a delivery, an invoice and the journal entry it posts - and moving the order's delivery status. Three layers need these (the BAPIs, the IDoc generators, an inbound delivery) and a document must look the same whichever created it. |
...
| `test_annotations.py` | The UI vocabulary: header, columns, filter bar, field groups, facets and capabilities in both encodings, that every annotated path is a real property and every facet target resolves, and that the V2 services keep their `sap:` attributes instead. |
| `test_v2_annotations.py` | The annotation document: that it is discoverable from the service document, references the vocabulary and the service, does not nest `Annotations` blocks, names only real properties and navigations, and that the V2 and V4 routes stay separate. |
| `test_documents.py` | The order-to-cash chain: that the seeded documents reference what they came from, that invoices add up and journal entries balance, delivery creation and its refusals, accounting postings and the balance check, and that an IDoc leaves a readable document behind. |
